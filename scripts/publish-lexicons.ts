...
import { DEV_INTROSPECT_URL } from "../lib/config";
...
async function main() {
  ...
  const docs = await loadLexicons(join(process.cwd(), "lexicons"));
...
async function getAuthority(): Promise<Authority> {
  ...
  try {
    introspectionResponse = await fetch(DEV_INTROSPECT_URL);
  } catch {
    throw new Error(
      `Cannot reach ${DEV_INTROSPECT_URL}. Start the atproto multi-PDS test network first.`,
    );
  }
